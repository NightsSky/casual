...
class GiteeService {
  static const _baseUrl = 'https://gitee.com/api/v5';
  ...
  Future<bool> testConnection(String token) async {
    final response = await http.get(
      Uri.parse('${_api('/user')}?access_token=$token'),
    );
  ...
  Future<List<Map<String, dynamic>>> listFiles({
    ...
    final url =
        '${_api('/repos/$owner/$repo/contents/$path')}?ref=$branch&access_token=$token';
  ...
  Future<Map<String, dynamic>> createOrUpdateFile({
    ...
    final url =
        '${_api('/repos/$owner/$repo/contents/$path')}?access_token=$token';
    ...
    // Gitee 创建文件使用 POST，更新已有文件必须携带 sha 并使用 PUT；
    // 否则服务端会按创建处理并返回“文件名已存在”。
    ...
    throw Exception(
        error['message'] ?? 'Gitee API Error: ${response.statusCode}');
  }
